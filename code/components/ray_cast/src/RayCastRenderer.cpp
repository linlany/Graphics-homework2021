...
namespace RayCast
{
    ...
    RGB RayCastRenderer::trace(const Ray& r) {
        ...
        if (closestHitObj) {
            ...
            KnnPhoton np;
            np.dis = 2;
            np.posi = hitRec.hitPoint;
            np.found = 0;
            myMap->getNPhoton(&np, 1);
            //myMap->getNPhoton(&np);
            int n = np.maxHeap.size();
            RGB co = Vec3{ 0 };
            for (int i = 0; i < 200 && i<n; i++) {
                Photon temp = np.maxHeap.top();
                np.maxHeap.pop();

                auto out = - glm::normalize(temp.direction);
    ...
    void RayCastRenderer::tracePhoton(Ray& r, Vec3 power, int depth = 0) {
        auto closestHitObj = closestHit(r);
        if (closestHitObj) {
            auto& hit = *closestHitObj;
            if (depth < 5) {
                //·´Éä´ý¼ÓÈë
                Photon pn;
                pn.direction = r.direction;
                pn.position = hit.hitPoint;
                pn.power = power;
                myMap->storePhoton(pn);
            }
        }
    }

    void RayCastRenderer::genePhotonMap() {
        auto& l = scene.pointLightBuffer[0];
        Vec3 ori = l.position;
        Vec3 power = l.intensity;
        Vec3 dir;
        power.x /= 100;
        power.y /= 100;
        power.z /= 100;
        for (int i = 0; i < myMap->maxNumber; i++) {
            this->myMap->generPhoton(dir, i);
            Ray r(ori, dir);
            tracePhoton(r, power);
        }
    }
